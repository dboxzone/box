...
case ${CMD} in
  "ide")
    ...
    docker-compose -f ${CODE_SERVER_CWD}/docker-compose.yml stop monitor code-server traefik >> ${CODE_SERVER_LOGS}/cs.log 2>&1
    docker-compose -f ${CODE_SERVER_CWD}/docker-compose.yml rm -f monitor code-server traefik >> ${CODE_SERVER_LOGS}/cs.log 2>&1
    sudo systemctl start code-server-ide >> ${CODE_SERVER_LOGS}/cs.log 2>&1
    docker-compose -f ${CODE_SERVER_CWD}/docker-compose.yml up -d monitor traefik code-server >> ${CODE_SERVER_LOGS}/cs.log 2>&1
    echo ""
