...
case ${CMD} in
  "ide")
    ...
    docker-compose -f ${CODE_SERVER_CWD}/docker-compose.yml stop dbox-apid code-server traefik >> ${CODE_SERVER_LOGS}/cs.log 2>&1
    docker-compose -f ${CODE_SERVER_CWD}/docker-compose.yml rm -f dbox-apid code-server traefik >> ${CODE_SERVER_LOGS}/cs.log 2>&1
